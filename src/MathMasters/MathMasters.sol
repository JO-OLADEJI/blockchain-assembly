...
library MathMasters {
    ...
    function mulWad(uint256 x, uint256 y) internal pure returns (uint256 z) {
        ...
        assembly {
            ...
            // @audit - I feel like the line below is problematic
            if mul(y, gt(x, div(not(0), y))) {
                // @audit-q why does `0xbac65e5b` not match the signature of `MathMasters__MulWadFailed -> 0xa56044f7` ??
                mstore(0x40, 0xbac65e5b) // `MathMasters__MulWadFailed()`.
                // @audit the wrong error selector is being returned
                revert(0x1c, 0x04)
            }
            z := div(mul(x, y), WAD)
        }
    ...
    function mulWadUp(uint256 x, uint256 y) internal pure returns (uint256 z) {
        ...
        assembly {
            ...
            // if iszero(
            //     sub(
            //         div(                // same thing as divide x by y -> `(x / y) - 1` == 0 && x++
            //             add(z, x),
            //             y
            //         ),
            //         1
