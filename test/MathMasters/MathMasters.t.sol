...
contract MathMastersTest is BaseTest {
    ...
    // unit test invalidating the logic in library
    function testMulWadUpUnit() public pure {
        uint256 x = 410051813495619480466613455;
        uint256 y = 243123422363753296511072251;

        uint256 libResult = MathMasters.mulWadUp(x, y);
        uint256 solResult = x * y == 0 ? 0 : (x * y - 1) / 1e18 + 1;

        console2.log("Lib. Result: ", libResult);
        console2.log("Test Result: ", solResult);
    }

    // Fuzz caught the following error
    // x -> 410051813495619480466613455
    // y -> 243123422363753296511072251
